...
export function Features() {
  ...
  return (
    <section id="features" className="bg-background px-4 py-32 border-b border-border">
      <div className="mx-auto max-w-7xl">
        <div className="mb-24 flex flex-col items-center text-center space-y-6">
          <p className="text-sm font-bold text-primary tracking-widest uppercase">The Toolkit</p>
          <h2 className="text-5xl font-playfair font-medium text-foreground sm:text-6xl tracking-tight">
            Uncompromising tools.<br/>
            <span className="italic font-light text-muted-foreground">For uncompromising professionals.</span>
          </h2>
        </div>
          ...
          {featuresList.map((feature, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative overflow-hidden rounded-2xl bg-card border border-border p-10 transition-shadow duration-300 hover:shadow-xl hover:bg-muted ${feature.colSpan}`}
              >
                <div className="relative z-10 flex h-full flex-col">
                  <div className="mb-8 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-sm transition-colors duration-300">
                    <Icon className="h-5 w-5 stroke-[2]" />
                  </div>
                  <div className="mt-auto">
                    <h3 className="font-semibold text-foreground text-2xl mb-3 font-playfair">{feature.title}</h3>
